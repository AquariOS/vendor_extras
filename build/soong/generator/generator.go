...
func init() {
	android.RegisterModuleType("aqua_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = aquaExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = aquaExpandVariables(ctx, dep_file)
	...
	cmd := aquaExpandVariables(ctx, String(g.properties.Cmd))
